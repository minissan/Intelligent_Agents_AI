...
def populate_objects(obstacles):
    ...
    for obs in obstacles:
        ...
        sim.setObjectPosition(shapeHandle, -1, [obs[0], obs[1], 0.1])
        sim.setObjectColor(shapeHandle,0,0,[1,0,0])
...
#MinMaxScaler
def scale_coordinates(a, X_min, X_max, Y_min, Y_max):
    X_std=(a-X_min)/(X_max-X_min)
    scaledValue=X_std*(Y_max-Y_min)+Y_min
    return scaledValue
...
def sysCall_init():
    ...
    #map scaling
    start = scale_coordinates(start[:-1], SimMin, SimMax, WallMin, WallMax ).astype(int)
    dest =  scale_coordinates(dest[:-1], SimMin, SimMax, WallMin, WallMax ).astype(int)
